"""Command-line delivery adapter for the upstream framework."""
...
from .runtime import ScientificRepresentationApp


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="scientific-representation",
        description="Portable equation-analysis to scientific-representation application",
    )
    parser.add_argument(
        "--root",
        type=Path,
        help="Framework root containing framework.json (defaults to this checkout)",
    )
    ...
    realize = subparsers.add_parser(
        "realize",
        help="Run raw input with importable Python lifecycle extensions",
        description=(
            "Run raw input through the lifecycle using importable Python "
            "extensions identified as package.module:ClassName."
        ),
    )
    realize.add_argument("raw_input", type=Path)
    direction = realize.add_mutually_exclusive_group()
    ...
    realize.add_argument(
        "--provider",
        required=True,
        metavar="PACKAGE.MODULE:CLASS",
        help="Python scientific-provider extension",
    )
    realize.add_argument(
        "--realizer",
        metavar="PACKAGE.MODULE:CLASS",
        help="Python target realizer extension for application integration",
    )
    realize.add_argument(
        "--reviewer",
        metavar="PACKAGE.MODULE:CLASS",
        help="Python reviewer extension for application integration",
    )
    realize.add_argument(
        "--target",
    ...
    account_direction = realize_account.add_mutually_exclusive_group()
    account_direction.add_argument("--direction")
    account_direction.add_argument("--direction-file", type=Path)
    question = realize_account.add_mutually_exclusive_group()
    ...
    realize_target.add_argument(
        "--stop-after",
        choices=("target_translation", "application"),
        default="application",
    )
    realize_target.add_argument("--output", type=Path, required=True)
    _json_flag(realize_target)

    realize_plan = subparsers.add_parser(
        "realize-plan",
        help="Resume application conformance from a persisted native plan",
        description="Resume application conformance from a persisted native plan.",
    )
    realize_plan.add_argument("plan_workspace", type=Path)
    realize_plan.add_argument("--target", required=True)
    realize_plan.add_argument("--realizer", required=True)
    realize_plan.add_argument("--output", type=Path, required=True)
    _json_flag(realize_plan)

    review = subparsers.add_parser(
        "review",
        help="Resume at independent scientific review",
        description="Resume at independent scientific review.",
    )
    review.add_argument("application_workspace", type=Path)
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        app = ScientificRepresentationApp(args.root)
...
def _dispatch(app: ScientificRepresentationApp, args: argparse.Namespace) -> dict[str, Any]:
    ...
    if args.command in {"build", "run"}:
        ...
        if args.command == "run":
            ...
            if not entry_point:
                raise FrameworkError("Built application declares no entry point")
    ...
    if args.command == "realize":
        ...
        if args.direction_file is not None:
            direction = _read_text(args.direction_file, "presentation direction")
    ...
    if args.command == "realize-account":
        ...
        return app.realize_account(
            ...
            args.output,
            presentation_direction=direction or "",
            presentation_question=presentation_question,
            provider=args.provider,
        )
    ...
    if args.command == "review":
        return app.review_application(
            args.application_workspace,
            args.output,
            target=args.target,
            reviewer=args.reviewer,
        )
    if args.command == "realize-plan":
        return app.realize_plan(
            args.plan_workspace,
            args.output,
            target=args.target,
            realizer=args.realizer,
...
def _print_human(command: str, result: dict[str, Any]) -> None:
    ...
    if command == "doctor":
        ...
        print(f"Framework root: {result['framework_root']}")
    ...
    if command in {
        ...
        print(f"Workflow workspace: {result['workspace']}")
        print(f"Completed stages: {', '.join(result['completed_stages'])}")
        print(
            "Status: "
            f"mechanical={result['status']['mechanical_conformance']}, "
            f"requirements={result['status']['target_requirement_coverage']}, "
            f"review={result['status']['scientific_review']}"
        )
